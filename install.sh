...
cmd_update() {
  ...
  # `up -d` leaves the container alone when the image did not change, so it
  # is safe to run either way and the version afterwards tells the truth.
  dc up -d
  wait_healthy || {
