...
| Control | Implementation |
|---|---|
| Instance model key | `instance_setting.ai_api_key`, set by the account marked `is_instance_admin` at `/admin/server`. `ANTHROPIC_API_KEY` seeds it once at startup when nothing is stored and never overwrites a key set through the page. It is instance-wide because it pays for every organization's calls; whether a given organization's content may be sent is a separate per-tenant opt-in. Stored so it can be replayed; see "Tokens" below. |
| Local owner password | Self-hosted instances sign in with `OWNER_USER` (default `admin`) and `OWNER_PASSWORD` instead of an identity provider. The account is identified by a username rather than a verified address, because this server issues the credential itself: the verified-address rule exists to distrust an identity provider's claim, and there is no provider here. It is scoped to `issuer = "local"` and every other issuer still requires one. The password is stored as argon2id (64 MiB, 3 passes, 2 lanes) with a per-credential salt, never in plaintext or as a fast hash. `POST /auth/local` is limited to 10 attempts per minute per IP, checks the origin, and answers an unknown username and a wrong password identically, having done the same work in both cases. It never creates an account: the owner is provisioned at startup. |
