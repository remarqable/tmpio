...
| `ANTHROPIC_API_KEY` | empty | Seeds the installation's model key at startup when none is stored. After that the key lives in `instance_setting` and is changed at `/admin/server`; this variable never overwrites it. Empty and nothing stored means rule-based filing only. Even with a key, nothing is sent for an organization until its owner turns filing on in Site settings; then the folder list, page titles and the first 2,500 bytes of the new text go to Anthropic and nothing else leaves the server. |
