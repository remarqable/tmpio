...
grep -q "Your site" "$W/dash.html" && ok "no organization name required (shows 'Your site')" || bad "site name fallback" "missing"
...
R=$(mcp "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"tmp_write\",\"arguments\":{\"path\":\"/research/circle.md\",\"content\":\"---\\ntitle: Circle\\ntags: [community]\\n---\\n\\n# Circle\\n\\n## Overview\\n\\nResearch text with [sources](https://example.com).\\n\\n:::callout type=\\\"note\\\"\\nWorking draft.\\n:::\\n\",\"expected_revision\":0,\"request_id\":\"$(uuid)\"}}}")
echo "$R" | grep -q '\\"revision\\": 1' && ok "tmp_write created /research/circle.md at revision 1" || bad "tmp_write" "$R"
code=$(curl -s -b "$J" -o /dev/null -w "%{http_code}" "$H/o:$ORG/research/circle"); check "canonical HTML URL renders for the owner" "$code" "200"
code=$(curl -s -o /dev/null -w "%{http_code}" "$H/o:$ORG/research/circle.md"); check "private read without credentials is refused (404)" "$code" "404"

echo "== 3. Create a writable secret link (owner browser only)"
curl -s -b "$J" -H "Origin: $H" -H "X-CSRF-Token: $CSRF" -H "Content-Type: application/json" -X POST "$H/api/v1/orgs/$ORG/share-links" -d '{"path":"/research/circle.md","label":"Reviewer A"}' > "$W/share.json"
...
R=$(mcp '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"tmp_read","arguments":{"path":"/research/circle.md"}}}')
...
R=$(mcp "{\"jsonrpc\":\"2.0\",\"id\":4,\"method\":\"tools/call\",\"params\":{\"name\":\"tmp_write\",\"arguments\":{\"path\":\"/research/circle.md\",\"content\":\"# Stale\\n\",\"expected_revision\":2,\"request_id\":\"$(uuid)\"}}}")
...
curl -s -b "$J" -H "Origin: $H" -H "X-CSRF-Token: $CSRF" "$H/api/v1/orgs/$ORG/history?path=/research/circle.md" > "$W/hist.json"
...
code=$(curl -s -b "$J" -o "$W/restore.json" -w "%{http_code}" -H "Origin: $H" -H "X-CSRF-Token: $CSRF" -H "If-Match: $ETAG" -H "Idempotency-Key: $(uuid)" -H "Content-Type: application/json" -X POST "$H/api/v1/orgs/$ORG/restores" -d '{"path":"/research/circle.md","revision":1}')
...
code=$(curl -s -b "$J" -o /dev/null -w "%{http_code}" -H "Origin: $H" -H "X-CSRF-Token: $CSRF" -H "If-Match: $ETAG" -H "Idempotency-Key: $(uuid)" -H "Content-Type: application/json" -X POST "$H/api/v1/orgs/$ORG/moves" -d '{"from":"/research/circle.md","to":"/notes/circle.md"}')
check "move /research/circle.md -> /notes/circle.md" "$code" "200"
code=$(curl -s -o /dev/null -w "%{http_code}" "$SURL/raw"); check "share link still reads after the move" "$code" "200"
loc=$(curl -s -b "$J" -o /dev/null -w "%{redirect_url}" "$H/research/circle"); check "old personal address redirects (form preserved)" "$loc" "$H/notes/circle"
loc=$(curl -s -b "$J" -o /dev/null -w "%{redirect_url}" "$H/o:$ORG/research/circle.md"); check "old explicit raw address redirects (form preserved)" "$loc" "$H/o:$ORG/notes/circle.md"
